<div>
    <x-modals.confirmation name="proceed-to-confirm-deposit">
        <x-slot name="title">
            <h2 class="text-left  text-xs font-bold tracking-wider">Proceed to confirmation?</h2>
        </x-slot>
        <x-slot name="body">
        ...
        <x-slot name="footer">
            ...
            <button wire:click="confirmdeposit" class="focus:border-green-300 focus:ring focus:ring-green-200 focus:outline-none focus:ring-opacity-50 disabled:opacity-25 transition ease-in-out duration-150 self-center text-xs rounded bg-green-400 text-white py-1 px-3">Confirm <i class="ml-2 fas fa-long-arrow-alt-right"></i></button>
        </x-slot>
    ...
    <div class="flex space-x-2">
        <div class="p-0.5 flex w-full sm:w-64 h-8 border bg-gray-100 border-gray-200 rounded">
    ...
    <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-6">
        @foreach ($responses->take(8) as $response) 
            <div class="flex flex-col shadow p-2 space-y-3 bg-white {{$activeWallet === $response['currency'] ? 'ring-1 ring-purple-500' : ''}}}">
                <div class="flex space-x-2">
    ...
    <div class="md:flex justify-between">
        <small class="self-center text-xs font-bold">If you have any questions, please <span class="block md:inline">go to the <b class="text-green-500">video representation section</b></span></small>
        <button onclick="$modals.show('proceed-to-confirm-deposit')" class="focus:border-green-300 focus:ring focus:ring-green-200 focus:outline-none focus:ring-opacity-50 mt-3 mt-0 w-full md:w-20 self-center text-xs rounded bg-green-400 text-white py-1 px-3">Next <i class="ml-2 fas fa-long-arrow-alt-right"></i></button>
    </div>
    <br>
</div>
