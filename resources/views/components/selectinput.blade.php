...
<div @click.away="open = false" class="cursor-pointer relative" x-data="{ open:false, activeItem: @entangle($attributes->wire('model')), items: {{ $items }} }">
    <template x-for="item in items" :key="item.id">
        <button @click="open = !open" x-show="activeItem === item.id" type="button" aria-haspopup="listbox" aria-expanded="true" aria-labelledby="listbox-label" class="cursor-pointer relative w-full bg-gray-100 pr-8 text-left cursor-default focus:outline-none sm:text-xs">
            <span class="flex items-left w-10 md:w-6" style="widt: 1.4rem">
    ...
    <div style="z-index: 99;display:none;" x-show.transition="open" x-transition:enter="transition ease-out duration-200" x-transition:enter-start="transform opacity-0 scale-95" x-transition:enter-end="transform opacity-100 scale-100" x-transition:leave="transition ease-in duration-75" x-transition:leave-start="transform opacity-100 scale-100" x-transition:leave-end="transform opacity-0 scale-95" class="absolute mt-1 w-full bg-white shadow-lg h-100 overflow-auto">
        <ul wire:click="updateCoin" tabindex="-1" role="listbox" aria-labelledby="listbox-label" aria-activedescendant="listbox-item-3" class="overflow-auto text-base ring-1 ring-black ring-opacity-5 overflow-auto focus:outline-none sm:text-sm">
            <template x-for="item in items" :key="item.id">
                <li :class="{ 'text-white bg-green-600': activeItem === item.id }" @click="activeItem = item.id, open = !open" id="listbox-item-0" role="option" class="cursor-default select-none relative py-2 pl-1 pr-9">
